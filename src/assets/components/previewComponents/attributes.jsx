function Attribute({
    ...
    renderAttributes = infoState.map((attribute) => {
      return (
        <div key={attribute.id}>
          ...
          <p>{attribute.university || attribute.companyName}</p>
          <p>{attribute.contry || attribute.description}</p>
        </div>
      );
    });
  }
  ...
  const content =
    type === "text" ? (
      <div className="text-area-div">{infoState[dataKey]}</div>
    ) : (
  ...
  return (
    <div
      className={
        isOutlined ? "top-border" + " attribute-list" : "attribute-list"
      }
    >
